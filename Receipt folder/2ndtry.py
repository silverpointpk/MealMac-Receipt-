...
# Format current time as "Placed At"
placed_time = datetime.now().strftime("%b %d %I:%M %p")

# Format confirmed pick-up time from JSON
pickup_raw = data["confirmed_pick_up_time"]  # e.g. "11 Mar 2025 18:30
pickup_dt = datetime.strptime(pickup_raw, "%d %b %Y %H:%M")
ready_at = datetime.now().strftime("%d %b %Y %I:%M %p")

# Now safely use in your order details
order_details = [
    ("Order #", str(data["order_number"])),
...
address_lines = [
    (restaurant["name"], bold_font_large, 30, height - 260),
...
address_lines = [
    (restaurant["address"], bold_font_medium, 30, height - 209),
...
address_lines = [
    (restaurant["phone"], bold_font_small, 30, height - 150),
